...
import {bfs, getNodesInShortestPathOrderBFS} from '../Algorithm/bfs';
...
class PathfindingVisualizer extends React.Component
{
    ...
    shortestPathVisualize(visitedaNodes,algo)
    {
        
      const node=visitedaNodes[visitedaNodes.length-1];
      let shortestPath=Array()
      shortestPath=getNodesInShortestPathOrder(node);
    ...
    shortestPathVisualizeBFS(visitedaNodes)
    {
        
      const node=visitedaNodes[visitedaNodes.length-1];
      let shortestPath=Array()
      shortestPath=getNodesInShortestPathOrderBFS(node);
    ...
    visualizeDijkstra()
    {
        ...
        let lastNode=visitedaNodes[visitedaNodes.length-1]
        if(lastNode.row!=FINISH_NODE_ROW || lastNode.col!=FINISH_NODE_COL)
          return
        setTimeout(()=>{
          this.shortestPathVisualize(visitedaNodes);
        },visitedaNodes.length*11);
    ...
    visualizeBFS()
    {
        ...
        const startNode=grid[START_NODE_ROW][START_NODE_COL];
        const endNode=grid[FINISH_NODE_ROW][FINISH_NODE_COL];
        const visitedaNodes=bfs(grid,startNode,endNode);
        console.log("visited nodes of bfs");
        console.log(visitedaNodes);
        this.animateDijkstra(visitedaNodes);
        setTimeout(()=>{
          this.shortestPathVisualizeBFS(visitedaNodes);
                  ...
                  <MDBNavbar color="default-color" dark expand="md">
                    ...
                    <MDBCollapse isOpen={this.state.collapse} navbar>
                      <MDBNavbarNav left>
                        ...
                        <MDBNavItem>
                          <MDBNavLink to="#" onClick={() => this.visualizeBFS()}>BFS Algo</MDBNavLink>
                        </MDBNavItem>
                      </MDBNavbarNav>
                      <MDBNavbarNav right>
                        <MDBNavItem>
